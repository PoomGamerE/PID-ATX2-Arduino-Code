...
void pid(float Kp , float Kd , int base_speed){
...
void pid_T(float Kp , float Kd , int base_speed, long mil){
  long a = millis();
  while (a + mil > millis())
  pid(Kp , Kd , base_speed);
}

void pid_FB(float Kp , float Kd , int base_speed){
  int bug = (ReadSensorMinValue(1) + ReadSensorMaxValue(1)) / 2
  while ((analog(1) < bug) && (analog(2) < bug) && (analog(3) < bug) && (analog(4) < bug) && (analog(5) < bug) && (analog(6) < bug))
  pid(Kp , Kd , base_speed);
  ao();
}


void pid_BB(float Kp , float Kd , int base_speed){
  while (analog(7) > 300 && analog(7) > 300)pid(Kp, Kd, base_speed);
  while (analog(5) < 300 || analog(0) < 300)pid(Kp, Kd, base_speed);
  ao();
  //pid_T(Kp,Kd,base_speed,100);
}

void gotokeep(float Kp , float Kd , int base_speed){
  while (1)
  {
    pid(Kp, Kd, base_speed);
    if ( getdist(6)<5 ){
